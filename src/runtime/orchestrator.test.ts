...
test("漏洞团队 spawn runtime agent 尚未落库时，getTaskSnapshot 不会把任务提前判 finished", async () => {
  ...
  const taskAgentNamesDuringDispatchWindow = typed.store
  ...
  const runtimeAgentName = revisionRequestMessage
    ? getMessageTargetAgentIds(revisionRequestMessage)[0] ?? null
    : null;

  assert.equal(typeof runtimeAgentName, "string");
  assert.equal(runtimeAgentName?.startsWith("反方-"), true);
  assert.equal(taskAgentNamesDuringDispatchWindow.includes(runtimeAgentName!), false);
