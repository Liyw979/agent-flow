...
test("applyTeamDsl 会一次性写入当前 Project 的 agents 与 topology", async () => {
  ...
  const compiled = compileTeamDsl({
    ...
    links: [
      { from: "BA", to: "Build", trigger_type: "handoff", message_type: "last" },
      { from: "Build", to: "SecurityResearcher", trigger_type: "handoff", message_type: "last" },
      { from: "SecurityResearcher", to: "Build", trigger_type: "action_required", message_type: "last" },
    ],
  });
  ...
  assert.deepEqual(updated.topology.edges, [
    { source: "BA", target: "Build", triggerOn: "handoff", messageMode: "last" },
    { source: "Build", target: "SecurityResearcher", triggerOn: "handoff", messageMode: "last" },
    {
...
test("保存拓扑后不会再生成旧工作区快照文件", async () => {
  ...
  const saved = await orchestrator.saveTopology({
    ...
    topology: {
      ...project.topology,
      nodes: ["BA", "Build"],
      edges: [{ source: "BA", target: "Build", triggerOn: "handoff", messageMode: "last" }],
    },
  });
...
test("保存拓扑后会把动态 spawn 团队配置保留在当前运行时快照里", async () => {
  ...
  const saved = await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [{ source: "Build", target: "初筛", triggerOn: "handoff", messageMode: "last" }],
      nodeRecords: [
...
test("保存拓扑后会保留 spawnEnabled 标记，避免 GUI 点击后回读丢失", async () => {
  ...
  const saved = await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [{ source: "Build", target: "UnitTest", triggerOn: "handoff", messageMode: "last" }],
      nodeRecords: [
        { id: "Build", kind: "agent", templateName: "Build" },
    ...
    getEdgeMessageMode: (
      topology: {
        edges: Array<{
          source: string;
          target: string;
          triggerOn: "handoff" | "approved" | "action_required";
          messageMode: "none" | "last" | "all";
        }>;
      },
      sourceAgentId: string,
      targetAgentId: string,
      triggerOn: "handoff" | "approved" | "action_required_request",
    ) => "none" | "last" | "all";
    buildAgentExecutionPrompt: (prompt: {
      mode: "structured";
      from: string;
...
test("修复首个失败 reviewer 后，Build 下一轮不会立刻全量重派全部 reviewer", async () => {
  ...
  await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [
        { source: "Build", target: "UnitTest", triggerOn: "handoff", messageMode: "last" },
        { source: "Build", target: "TaskReview", triggerOn: "handoff", messageMode: "last" },
        { source: "Build", target: "CodeReview", triggerOn: "handoff", messageMode: "last" },
        { source: "UnitTest", target: "Build", triggerOn: "action_required", messageMode: "last" },
        { source: "TaskReview", target: "Build", triggerOn: "action_required", messageMode: "last" },
        { source: "CodeReview", target: "Build", triggerOn: "action_required", messageMode: "last" },
      ],
    },
  });
...
test("审查 Agent 返回 action_required 后会在其余 reviewer 收齐后回流到 Build", async () => {
  ...
  const runningSnapshot = await waitForTaskSnapshot(
    ...
    (snapshot) =>
      ...
      && unitTestRunCount === 1
      && buildRunCount === 1
      && snapshot.agents.some((agent) => agent.name === "UnitTest" && agent.status === "action_required"),
  );
  assert.equal(runningSnapshot.task.status, "running");
  assert.equal(buildRunCount, 1);
  assert.equal(
    runningSnapshot.agents.some((agent) => agent.name === "UnitTest" && agent.status === "action_required"),
    true,
  );
  ...
  const displayContent = (
    orchestrator as unknown as {
      createDisplayContent: (
        parsedReview: {
          cleanContent: string;
          decision: "approved" | "action_required" | "invalid";
          opinion: string | null;
...
test("审视不通过且只返回 action_required 标签时，群聊展示会去掉标签", () => {
  const orchestrator = createTestOrchestrator({
    userDataPath: createTempDir(),
    enableEventStream: false,
  });
  ...
  const typed = orchestrator as unknown as Orchestrator & {
    ...
    store: {
      updateTaskAgentStatus: (
        ...
        taskId: string,
        agentName: string,
        status: "idle" | "completed" | "failed" | "running" | "action_required",
      ) => void;
    };
  };
  stubOpenCodeAttachBaseUrl(orchestrator);
...
test("聊天页面会按每条 action_required 边的单独上限展示失败原因", async () => {
  ...
  await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [
        { source: "Build", target: "UnitTest", triggerOn: "handoff", messageMode: "last" },
        { source: "UnitTest", target: "Build", triggerOn: "action_required", maxRevisionRounds: 2, messageMode: "last" },
      ],
    },
  });
