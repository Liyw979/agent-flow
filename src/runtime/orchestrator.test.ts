...
import { compileBuiltinTopology, compileBuiltinVulnerabilityTopology } from "./builtin-topology-test-helpers";
    ...
    nodes: [
      {
        type: "agent",
        id: "线索发现",
        prompt: "你是线索发现。请在完成时返回 <complete>。",
        writable: false,
      },
      {
        type: "agent",
        id: "漏洞讨论",
        prompt: "你是漏洞讨论。请在完成时返回 <complete>。",
        writable: false,
      },
      {
        type: "agent",
      ...
      nodes: [
        {
          type: "agent",
          id: "A",
          prompt: "你是 A。请在完成时返回 <complete>。",
          writable: false,
        },
        {
          type: "agent",
          id: "B",
          prompt: "你是 B。请在完成时返回 <complete>。",
          writable: false,
        },
        {
          type: "agent",
  ...
  state.runtimeNodes = [
    {
      ...
      displayName: "漏洞挑战-1",
      sourceNodeId: "线索发现",
      groupId: "spawn-rule:疑点辩论:finding-001",
      role: "漏洞挑战",
    },
    {
      id: "漏洞论证-1",
    ...
    },
    {
      ...
      displayName: "漏洞挑战-2",
      sourceNodeId: "线索发现",
      groupId: "spawn-rule:疑点辩论:finding-002",
      role: "漏洞挑战",
    },
    {
      id: "漏洞论证-2",
...
test("rfc-scanner 的 spawn 首轮派发到漏洞论证时，会额外注入来自线索发现的 initialMessage", async () => {
  ...
  await orchestrator.applyTeamDsl({
    cwd: projectPath,
    compiled: compileBuiltinTopology("rfc-scanner.json5"),
  });
  ...
  await waitForTaskSnapshot(
    orchestrator,
    submittedTask.task.id,
    () => (promptByAgent.get("漏洞论证")?.length ?? 0) >= 1,
  );
  ...
  const storedMessages = orchestrator.store
    .listMessages(projectPath, submittedTask.task.id)
    .map((message) => ({
  ...
  assert.deepEqual(
    ...
    ["[From 漏洞挑战-1 Agent]", "[From 线索发现 Agent]"],
    JSON.stringify({ firstPrompt, storedMessages }, null, 2),
  );
  await orchestrator.dispose({
    awaitPendingTaskRuns: false,
