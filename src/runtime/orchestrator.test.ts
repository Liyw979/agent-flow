...
test("保存拓扑后会把动态 spawn 团队配置保留在当前运行时快照里", async () => {
  ...
  const saved = await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [{ source: "Build", target: "初筛", triggerOn: "handoff" }],
      nodeRecords: [
...
test("保存拓扑后会保留 spawnEnabled 标记，避免 GUI 点击后回读丢失", async () => {
  ...
  const saved = await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [{ source: "Build", target: "UnitTest", triggerOn: "handoff" }],
      nodeRecords: [
        { id: "Build", kind: "agent", templateName: "Build" },
...
test("聊天页面会按每条 action_required 边的单独上限展示失败原因", async () => {
  ...
  await orchestrator.saveTopology({
    ...
    topology: {
      ...
      edges: [
        { source: "Build", target: "UnitTest", triggerOn: "handoff" },
        { source: "UnitTest", target: "Build", triggerOn: "action_required", maxRevisionRounds: 2 },
      ],
    },
  });
