...
test("agent-final 已包含继续处理正文时，合并 continue-request 不会重复追加同一段回应", () => {
  ...
  const merged = mergeTaskChatMessages([
    ...
    createMessage({
      ...
      kind: "continue-request",
      followUpMessageId: "agent-final-with-body-and-response",
...
test("decisionAgent 的回流消息即使被其他消息隔开，也应继续合并回原结果卡片", () => {
  ...
  const merged = mergeTaskChatMessages([
    ...
    createMessage({
      ...
      kind: "continue-request",
      followUpMessageId: "task-decision-final",
      targetAgentIds: ["Build"],
    }),
  ]);
...
test("agent-final 已包含继续处理正文时，即使被其他消息隔开，回流消息也不会重复追加同一段回应", () => {
  ...
  const merged = mergeTaskChatMessages([
    ...
    createMessage({
      ...
      content: `${decisionBody}\n\n@Build`,
      kind: "continue-request",
      followUpMessageId: "task-decision-final-with-body-and-response",
      targetAgentIds: ["Build"],
    }),
  ]);
...
test("all_completed 辩论里 complete 后立刻补发的 continue-request 也应合并回原结果卡片", () => {
  const finalBody = "这个可疑点成立，属于真实的鉴权缺口，不是误报。";

  const merged = mergeTaskChatMessages([
    createMessage({
      id: "challenge-final-complete",
      sender: "漏洞挑战-4",
      timestamp: "2026-04-25T06:46:03.997Z",
      content: finalBody,
      kind: "agent-final",
      decision: "complete",
    }),
    createMessage({
      id: "challenge-followup-continue-request",
      sender: "漏洞挑战-4",
      timestamp: "2026-04-25T06:46:04.025Z",
      content: `${finalBody}\n\n@漏洞论证-4`,
      kind: "continue-request",
      followUpMessageId: "challenge-final-complete",
      targetAgentIds: ["漏洞论证-4"],
    }),
  ]);
...
test("continue-request 的 followUpMessageId 对不上时，即使 sender 相同也不能误合并", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      id: "challenge-final-complete",
      sender: "漏洞挑战-4",
    ...
    createMessage({
      id: "challenge-followup-continue-request-wrong-link",
      sender: "漏洞挑战-4",
      timestamp: "2026-04-25T06:46:04.025Z",
      content: "第二条继续请求。\n\n@漏洞论证-4",
      kind: "continue-request",
      followUpMessageId: "another-agent-final",
      targetAgentIds: ["漏洞论证-4"],
    }),
  ]);
...
test("continue-request 单独展示时会移除标签后再追加 mention", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      ...
      kind: "continue-request",
      followUpMessageId: "standalone-agent-final",
      targetAgentIds: ["Build"],
    }),
  ]);
...
test("action_required 判定正文命中结论标题时，聊天展示不能只截取最后的结论章节", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      id: "agent-final-needs-continue-heading",
      sender: "漏洞挑战-1",
