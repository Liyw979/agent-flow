...
test("decisionAgent 的回流消息即使被其他消息隔开，也应继续合并回原结果卡片", () => {
  ...
  assert.deepEqual(merged[0]?.kinds, ["agent-final", "action-required-request"]);
...
test("agent-final 展示时会保留非结构化孤立结束标签", () => {
  ...
  assert.equal(merged.length, 1);
  assert.equal(merged[0]?.content, `${EXAMPLE_TRIGGER_END_LABEL}继续处理。\n\n请补充实现依据。`);
});
...
给定 a = 1、b = 2 时，返回 c = 3`;
  ...
  const merged = mergeTaskChatMessages([
    createDefaultAgentFinalMessage("agent-final", summary, DEFAULT_SENDER, DEFAULT_TIMESTAMP),
    createAgentDispatchMessage("agent-dispatch", "@Build", ["Build"], "@Build", DEFAULT_SENDER, DEFAULT_TIMESTAMP),
  ]);
