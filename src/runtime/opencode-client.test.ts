...
test("resolveExecutionResult 在 completed 响应未返回合法 trigger 时会按 60 秒间隔重试直到拿到合法 trigger", async () => {
  ...
  typed.waitForMessageCompletion = async (_target, _sessionId, messageId) => ({
    id: `reply-${messageId}`,
    content: firstReply
      ? (firstReply = false, "<456> 非法判定")
      : "<continue>第二次恢复</continue>",
    sender: "assistant",
  ...
  assert.equal(result.finalMessage, "<continue>第二次恢复</continue>");
  assert.deepEqual(submittedContents, [
    "初始请求",
    "需要返回：<continue> / <complete>",
  ]);
  const submittedRetryAt = submittedAt as [number, number];
  assert.deepEqual([submittedRetryAt[1] - submittedRetryAt[0]], [60_000]);
});
