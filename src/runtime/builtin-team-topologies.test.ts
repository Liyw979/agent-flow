...
test("开发团队拓扑包含 CodeReview 审查回路", () => {
  ...
  assert.equal(
    developmentTeamTopology.links.some(
      (link: BuiltinLink) => link.from === "Build" && link.to === "CodeReview" && link.trigger_type === "handoff",
    ),
    true,
  );
  assert.equal(
    developmentTeamTopology.links.some(
      (link: BuiltinLink) => link.from === "CodeReview" && link.to === "Build" && link.trigger_type === "action_required",
    ),
    true,
  );
});
...
test("漏洞挖掘团队默认使用正反双方多轮对弈，而不是固定四个辩手串行两轮", () => {
  ...
  assert.equal(spawnRule?.reportToTriggerOn, "handoff");
...
test("漏洞团队的裁决总结 prompt 约束漏洞输出报告、误报不额外输出，并按 handoff 回到初筛", () => {
  const vulnerabilityTeamTopology = readBuiltinTopology("vulnerability-team.topology.json");
  const debateNode = vulnerabilityTeamTopology.nodes.find((node: BuiltinNode) => node.name === "疑点辩论");
