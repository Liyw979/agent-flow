...
test("本项目内提供开发团队与漏洞挖掘团队拓扑文件", () => {
  assert.equal(fs.existsSync(path.join(BUILTIN_TOPOLOGY_DIR, "development-team.topology.json5")), true);
  assert.equal(fs.existsSync(path.join(BUILTIN_TOPOLOGY_DIR, "single-agent-ba.topology.json5")), true);
  assert.equal(fs.existsSync(path.join(BUILTIN_TOPOLOGY_DIR, "vulnerability.json5")), true);
...
test("开发团队拓扑为每个 decisionAgent 显式声明回流与成功 trigger", () => {
  const topology = readBuiltinTopology("development-team.topology.json5");
...
test("单 Agent 示例拓扑也使用递归式 DSL 并可直接编译", () => {
  const topology = readBuiltinTopology("single-agent-ba.topology.json5");
...
test("漏洞挖掘团队默认使用论证与挑战多轮对弈，而不是固定四个辩手串行两轮", () => {
  const topology = readBuiltinTopology("vulnerability.json5");
  const debateNode = topology.nodes.find((node) => node.id === "疑点辩论");
  assert.ok(debateNode);
  assert.equal(debateNode.type, "spawn");
  assert.deepEqual(
    debateNode.graph.links.map((link) => ({ from: link.from, to: link.to, trigger: link.trigger })),
    [
