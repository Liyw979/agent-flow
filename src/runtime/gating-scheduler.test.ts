...
test("all_completed 子图里的 approved 多入边必须全部命中后才会派发目标节点", () => {
  const topology: TopologyRecord = {
    ...
    nodeRecords: [
      { id: "漏洞论证-1", kind: "agent", templateName: "漏洞论证" },
      { id: "漏洞挑战-1", kind: "agent", templateName: "漏洞挑战" },
      { id: "讨论总结-1", kind: "agent", templateName: "讨论总结" },
    ],
    edges: [
      { source: "漏洞论证-1", target: "讨论总结-1", triggerOn: "complete", messageMode: "last" },
  ...
  const firstPlan = firstScheduler.planApprovedDispatch(
    "漏洞论证-1",
    "漏洞论证同意进入裁决",
    [
  ...
  const secondPlan = secondScheduler.planApprovedDispatch(
    "漏洞论证-1",
    "漏洞论证已在对方回应后完成本轮总结前判断",
    [
      { id: "漏洞论证-1", status: "completed" as const },
      { id: "漏洞挑战-1", status: "continue" as const },
      { id: "讨论总结-1", status: "idle" as const },
    ],
  );
