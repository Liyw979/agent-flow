...
test("边配置为 none 时，下游只收到原始标签正文，不再携带上游最后一条正文", () => {
  const messages = [
    createUserMessage({
      ...
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
...
test("边配置为 none 且 initialMessage 包含当前 source 时，会注入该来源最后一条可转发消息", () => {
  const messages = [
    createUserMessage({
      content: "@线索发现 请继续分析。",
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
...
test("initialMessage 包含当前 source 且最后一条与当前正文相同，会去重为单个来源段落", () => {
  const messages = [
    createUserMessage({
      content: "@漏洞讨论 请继续分析。",
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
  ...
  assert.deepEqual(forwarded, {
    kind: "forwarded",
    agentMessage: "[From 漏洞讨论 Agent]\n漏洞讨论当前结论。",
  });
});

test("initialMessage 会优先取来源 agent 的最后一条 action-required-request 正文", () => {
  const messages = [
    createUserMessage({
      content: "@漏洞讨论 请继续分析。",
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
    ...
    createActionRequiredRequestMessage({
      sender: "漏洞讨论",
      content: "请继续补充第二条证据。\n\n@Build",
      timestamp: toUtcIsoTimestamp("2026-04-16T00:00:01.000Z"),
      targetAgentIds: ["Build"],
...
test("首条 agent-progress 早于 agent-final 时，initialMessage 仍应注入 agent-final 正文", () => {
  const messages = [
    ...
    createAgentProgressMessage({
      sender: "漏洞讨论",
      content: "正在读取文件并整理线索。",
      timestamp: toUtcIsoTimestamp("2026-04-16T00:00:00.500Z"),
    }),
    createAgentFinalMessage({
      sender: "漏洞讨论",
      content: "漏洞讨论首条证据。",
...
test("initialMessage 为多个 agent 时，下游会按编译后的来源顺序收到多个来源段落", () => {
  const messages = [
    ...
    createAgentFinalMessage({
      sender: "线索发现",
      content: "线索发现后续又发了一条，应覆盖更早的 initialMessage。",
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
      routingKind: "default",
    }),
  ];
  ...
  assert.deepEqual(forwarded, {
    ...
    [
      "[From 线索发现 Agent]",
      "线索发现后续又发了一条，应覆盖更早的 initialMessage。",
      "",
      "[From 漏洞讨论 Agent]",
      "线索完备性评估准备接手。",
      "",
...
test("initialMessage 可以额外注入另一个来源 agent 的最后一条可转发消息", () => {
  const messages = [
    createUserMessage({
      content: "@Build 初始任务：实现加法工具",
      timestamp: toUtcIsoTimestamp(TEST_TIMESTAMP),
...
test("initialMessage 写模板名时，会命中实例消息中的最后一条正文", () => {
  const messages = [
    createAgentFinalMessage({
      sender: "漏洞挑战",
      content: "漏洞挑战模板名对应的较早结论。",
      timestamp: toUtcIsoTimestamp("2026-05-01T00:00:00.000Z"),
      routingKind: "default",
    }),
    createAgentFinalMessage({
      sender: "漏洞论证-1",
      content: "漏洞论证-1 的当前结论。",
      timestamp: toUtcIsoTimestamp("2026-05-01T00:00:01.000Z"),
      routingKind: "default",
    }),
    createAgentFinalMessage({
      sender: "漏洞挑战-3",
      content: "漏洞挑战实例对应的最后结论。",
      timestamp: toUtcIsoTimestamp("2026-05-01T00:00:02.000Z"),
      routingKind: "default",
    }),
  ];
...
test("默认转发只保留当前来源消息", () => {
  const messages = [
    ...
    createAgentFinalMessage({
      ...
      timestamp: toUtcIsoTimestamp("2026-05-07T03:00:00.000Z"),
      routingKind: "default",
    }),
  ];
...
test("group 首轮派发给漏洞论证实例时，initialMessage 会补入静态线索发现消息", () => {
  const messages = [
    createUserMessage({
...
test("group 运行时实例刚被 dispatch 但尚未完成时，持久化补偿逻辑不会提前把任务判 finished", () => {
  const topology = createTopologyForTest({
    nodes: ["线索发现", "疑点辩论"],
    edges: [
      {
...
test("group 运行时实例已写入 dispatch 消息但尚未落库为 task agent 时，持久化补偿逻辑不会提前把任务判 finished", () => {
  const topology = createTopologyForTest({
    nodes: ["线索发现", "疑点辩论"],
    edges: [
      {
...
test("decisionAgent 仍处于 action_required 状态时，持久化补偿逻辑不会把中途流程误判为 finished", () => {
  ...
  const messages = [
    createAgentFinalMessage({
      ...
      timestamp: toUtcIsoTimestamp("2026-04-24T15:36:15.000Z"),
      routingKind: "default",
    }),
    createAgentDispatchMessage({
      sender: "Build",
      ...
      timestamp: toUtcIsoTimestamp("2026-04-24T15:36:16.000Z"),
    }),
    createActionRequiredRequestMessage({
      sender: "CodeReview",
      content: "还需要继续修改。\n\n@Build",
      ...
      targetAgentIds: ["Build"],
      timestamp: toUtcIsoTimestamp("2026-04-24T15:36:29.000Z"),
    }),
    createAgentFinalMessage({
      sender: "UnitTest",
      content: "测试通过。",
      timestamp: toUtcIsoTimestamp("2026-04-24T15:37:20.000Z"),
      routingKind: "default",
    }),
    createAgentFinalMessage({
      sender: "TaskReview",
      content: "可以验收。",
      timestamp: toUtcIsoTimestamp("2026-04-24T15:37:21.000Z"),
      routingKind: "default",
    }),
  ];
...
test("最新一条是 agent-dispatch 时，持久化补偿逻辑不会把重新派发中的任务误判为 finished", () => {
  ...
  const messages = [
    createAgentFinalMessage({
      ...
      timestamp: toUtcIsoTimestamp("2026-04-24T15:37:17.000Z"),
      routingKind: "default",
    }),
    createAgentDispatchMessage({
      sender: "Build",
