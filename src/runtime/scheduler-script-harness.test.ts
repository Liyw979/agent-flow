...
test("脚本模式支持用户首条直接 @reviewer 并沿 action_required 回路继续推进", async () => {
  const topology = createTopology({
    ...
    downstream: {
      安全负责人: { 漏洞分析人员: "action_required" },
      漏洞分析人员: { 安全负责人: "action_required" },
    },
  });
  const script = [
...
test("通用调度脚本模式支持非 Build 的实现者反复修复 reviewer 意见", async () => {
  const topology = createTopology({
    ...
    downstream: {
      ...
      UnitTest: { Implementer: "action_required" },
    },
  });
...
test("通用调度脚本模式支持 reviewer 通过后显式触发 approved 下游", async () => {
  const topology = createTopology({
    ...
    downstream: {
      BA: { Implementer: "handoff" },
      Implementer: { CodeReview: "handoff" },
      CodeReview: {
        Implementer: "action_required",
        TaskReview: "approved",
        UnitTest: "approved",
      },
    },
...
test("脚本首条 user 显式 @ 的 Agent 只要存在于 topology.nodes 就允许作为起点", async () => {
  const topology = createTopology({
    ...
    nodes: ["BA"],
    downstream: {
      Implementer: { TaskReview: "handoff" },
    },
  });
...
test("当前调度不支持在 action_required 对弈中由发起方直接跳到裁决", async () => {
  const topology = createTopology({
    ...
    downstream: {
      初筛: { 反方: "handoff" },
      正方: { 反方: "action_required", 裁决: "approved" },
      反方: { 正方: "action_required", 裁决: "approved" },
      裁决: { 初筛: "approved" },
    },
  });
...
test("BA dispatches Build through three review passes before the task can finish", async () => {
  const topology = createTopology({
    ...
    edges: [
      createEdge("BA", "Build", "handoff"),
      createEdge("Build", "UnitTest", "handoff"),
      createEdge("UnitTest", "Build", "action_required"),
      createEdge("UnitTest", "CodeReview", "approved"),
      createEdge("CodeReview", "Build", "action_required"),
      createEdge("CodeReview", "TaskReview", "approved"),
      createEdge("TaskReview", "Build", "action_required"),
    ],
  });
...
test("长链路脚本会覆盖 UnitTest、TaskReview、CodeReview 多轮往返后的最终双确认", async () => {
  const topology = createTopology({
    ...
    edges: [
      createEdge("Build", "UnitTest", "handoff"),
      createEdge("Build", "TaskReview", "handoff"),
      createEdge("Build", "CodeReview", "handoff"),
      createEdge("UnitTest", "Build", "action_required"),
      createEdge("TaskReview", "Build", "action_required"),
      createEdge("CodeReview", "Build", "action_required"),
    ],
