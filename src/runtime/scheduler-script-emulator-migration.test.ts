...
function buildSpawnItemsBody(...titles: string[]): string {
...
test("漏洞挖掘团队脚本支持论证挑战对弈超限后自动转给讨论总结", async () => {
  ...
  const script = [
    ...
    `线索发现: ${buildSpawnItemsBody("发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。")} @漏洞挑战-1`,
...
test("漏洞挖掘团队脚本里讨论总结完成后会回到线索发现继续挖掘", async () => {
  ...
  const script = [
    ...
    `线索发现: ${buildSpawnItemsBody("发现第 2 个可疑点：内部调试接口似乎缺少鉴权。")} @漏洞挑战-2`,
...
test("漏洞挖掘团队脚本里线索完备性评估要求继续时，会把具体补查方向回给线索发现", async () => {
  ...
  const script = [
    ...
    `线索发现: ${buildSpawnItemsBody("发现第 2 个可疑点：内部任务执行入口似乎允许未授权触发高危操作。")} @漏洞挑战-2`,
...
test("漏洞挖掘团队脚本不再接受非法短别名", async () => {
  ...
  const script = [
    "user: @线索发现 请持续挖掘当前代码中的可疑漏洞点。",
    `线索发现: ${buildSpawnItemsBody("发现第 1 个可疑点：上传文件名可能被直接拼进目标路径。")} @漏洞挑战-alias1`,
    ...
    `线索发现: ${buildSpawnItemsBody("发现第 2 个可疑点：内部调试接口似乎缺少鉴权。")} @漏洞挑战-alias2`,
...
test("script 模板支持 spawn 实例短名 sender 和 @target", async () => {
  ...
  const script = [
    "user: @Build 请完成这个需求",
    `Build: ${buildSpawnItemsBody("第 1 轮实现完成")} @TaskReview-1`,
