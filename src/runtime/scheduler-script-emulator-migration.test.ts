...
test("rfc-scanner 拓扑里的双向 <continue> 对弈在第 5 轮会转给讨论总结，避免团队停住", async () => {
  ...
  assert.deepEqual(escalationStep.afterDecision.batch.source, { kind: "agent", agentId: "漏洞挑战-1" });
...
test("漏洞挖掘团队脚本里讨论总结完成后会回到线索发现继续挖掘", async () => {
  ...
  const script = [
    ...
    "漏洞论证-1: <agree>第二轮补证：存储层直接执行 path.join(uploadRoot, filename)，未见对 .. 或分隔符的拦截。</agree> @讨论总结-1",
    "讨论总结-1: 当前这条更像真实漏洞。 @线索发现",
    "线索发现: <continue>发现第 2 个可疑点：内部调试接口似乎缺少鉴权。 @漏洞挑战-2",
...
test("漏洞挖掘团队脚本里线索完备性评估要求继续时，会把具体补查方向回给线索发现", async () => {
  ...
  const script = [
    ...
    "线索发现: <continue>发现第 2 个可疑点：内部任务执行入口似乎允许未授权触发高危操作。 @漏洞挑战-2",
...
test("漏洞挖掘团队脚本允许挑战侧单边 <agree> 直接进入讨论总结", async () => {
  ...
  const script = [
    "user: @线索发现 请持续挖掘当前代码中的可疑漏洞点。",
    "线索发现: <continue>发现第 1 个可疑点：内部调试接口似乎缺少鉴权。 @漏洞挑战-1",
    "漏洞挑战-1: <agree>当前材料已经足够进入总结。</agree> @讨论总结-1",
...
test("漏洞挖掘团队脚本不再接受非法短别名", async () => {
  ...
  const script = [
    ...
    "漏洞挑战-alias2: <agree>当前材料更像误报，可以进入裁决。</agree> @讨论总结-alias2",
