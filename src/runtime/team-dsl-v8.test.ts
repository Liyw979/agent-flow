...
test("compileTeamDsl 支持 v8 递归式图 DSL，并固定使用 items 字段展开 spawn", () => {
  const compiled = compileTeamDsl({
    ...
    nodes: [
      ...
      },
      {
        ...
        graph: {
          ...
          links: [
            { from: "正方", to: "反方", trigger_type: "action_required", message_type: "last" },
            { from: "反方", to: "正方", trigger_type: "action_required", message_type: "last" },
    ...
    links: [
      { from: "初筛", to: "辩论", trigger_type: "handoff", message_type: "last" },
      { from: "辩论", to: "初筛", trigger_type: "handoff", message_type: "last" },
    ],
  });
