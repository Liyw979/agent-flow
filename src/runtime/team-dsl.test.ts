...
function link(
  from: string,
  to: string,
  trigger_type: "handoff" | "approved" | "action_required",
...
function createDevelopmentGraphDsl() {
  return {
    ...
    links: [
      link("BA", "Build", "handoff", "last"),
      link("Build", "CodeReview", "handoff", "last"),
      link("Build", "UnitTest", "handoff", "last"),
      link("Build", "TaskReview", "handoff", "last"),
      link("CodeReview", "Build", "action_required", "last"),
      link("UnitTest", "Build", "action_required", "last"),
      link("TaskReview", "Build", "action_required", "last"),
    ],
  };
}
...
test("compileTeamDsl 支持把递归式图 DSL 编译成 agents + topology", () => {
  const compiled = compileTeamDsl({
    ...
    links: [
      link("BA", "Build", "handoff", "last"),
      link("Build", "SecurityResearcher", "handoff", "last"),
      link("SecurityResearcher", "Build", "action_required", "last"),
    ],
  });
  ...
  assert.deepEqual(compiled.topology.edges, [
    ...
    { source: "SecurityResearcher", target: "Build", triggerOn: "action_required", messageMode: "last" },
  ]);
...
test("compileTeamDsl 支持定义漏洞挖掘团队的正反多轮 spawn 辩论拓扑", () => {
  const compiled = compileTeamDsl({
    ...
    nodes: [
      ...
      spawnNode(
        "疑点辩论",
        {
          ...
          links: [
            link("正方", "反方", "action_required", "last"),
            link("反方", "正方", "action_required", "last"),
    ...
    links: [
      link("初筛", "疑点辩论", "handoff", "last"),
      link("疑点辩论", "初筛", "handoff", "last"),
    ],
  });
  ...
  assert.deepEqual(compiled.topology.edges, [
    { source: "初筛", target: "疑点辩论", triggerOn: "handoff", messageMode: "last" },
    { source: "疑点辩论", target: "初筛", triggerOn: "handoff", messageMode: "last" },
  ]);
  ...
  assert.equal(compiled.topology.spawnRules?.[0]?.reportToTriggerOn, "handoff");
...
test("compileTeamDsl 支持在 links 上显式声明边级消息传递策略", () => {
  const compiled = compileTeamDsl({
    ...
    links: [
      link("初筛", "疑点辩论", "handoff", "all"),
      link("疑点辩论", "裁决总结", "handoff", "last"),
      link("裁决总结", "初筛", "handoff", "none"),
    ],
  });

  assert.deepEqual(compiled.topology.edges, [
    { source: "初筛", target: "疑点辩论", triggerOn: "handoff", messageMode: "all" },
    { source: "疑点辩论", target: "裁决总结", triggerOn: "handoff", messageMode: "last" },
    { source: "裁决总结", target: "初筛", triggerOn: "handoff", messageMode: "none" },
  ]);
});
    ...
    matchesAppliedTeamDslTopology(
      {
        ...
        edges: [{ source: "Build", target: "BA", triggerOn: "handoff", messageMode: "last" }],
      },
      compiled,
    ),
    false,
