...
test("并发 reviewer 中单条回流链路超限时，不应提前打断其他 reviewer", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      ...
      { source: "Build", target: "TaskReview", triggerOn: "handoff", messageMode: "last" },
      { source: "Build", target: "CodeReview", triggerOn: "handoff", messageMode: "last" },
      { source: "UnitTest", target: "Build", triggerOn: "action_required", maxRevisionRounds: 1, messageMode: "last" },
...
test("回流超限时，如果 reviewer 正文已包含最终结论提示，不应再重复追加一遍", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "Build", target: "反方-1", triggerOn: "handoff", messageMode: "last" },
      { source: "反方-1", target: "Build", triggerOn: "action_required", messageMode: "last", maxRevisionRounds: 1 },
...
test("最后一个叶子节点完成后，router 会直接判定 finished，而不是错误停在 waiting", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "BA", target: "Build", triggerOn: "handoff", messageMode: "last" },
      { source: "Build", target: "QA", triggerOn: "handoff", messageMode: "last" },
    ],
  };
  const state = createGraphTaskState({
