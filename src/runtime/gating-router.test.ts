...
test("isExecutionDecisionAgent 会把 group 子图里带结束 trigger 出边的运行时实例识别为 decision agent", () => {
  ...
  assertGroupRulesValid(topology);
  ...
  state.runtimeNodes = [
    {
      ...
      displayName: "漏洞论证-1",
      sourceNodeId: "疑点辩论",
      groupId: "group-rule:疑点辩论:group-rule:疑点辩论-0001",
      role: "漏洞论证",
    },
    {
      id: "讨论总结-1",
...
test("isExecutionDecisionAgent 不会把没有任何非 <default> trigger 出边的普通 agent 误判为 decision agent", () => {
  const topology: TopologyRecord = withAgentNodeRecords({
    nodes: ["线索发现", "疑点辩论"],
    edges: [
      { source: "线索发现", target: "疑点辩论", trigger: "<default>", messageMode: "last" },
...
test("isExecutionDecisionAgent 会把仅通过 __end__ 暴露结束 trigger 分支的节点识别为 decision agent", () => {
  const topology: TopologyRecord = withAgentNodeRecords({
    nodes: ["线索发现"],
    edges: [],
    langgraph: {
...
test("用户消息命中 group 节点时会自动生成实例组并启动入口角色", () => {
  const topology: TopologyRecord = {
  ...
  assert.equal(state.groupBundles.length, 1);
  assert.equal(
...
test("自动 handoff 命中 group 节点时，会实例化动态团队并派发入口角色，而不是停在 group 模板节点", () => {
  const topology: TopologyRecord = {
    nodes: ["Build", "UnitTest", "TaskReview", "CodeReview"],
    nodeRecords: [
      ...
      { id: "TaskReview", kind: "group", templateName: "TaskReview", groupRuleId: "group-rule:TaskReview", groupEnabled: true, initialMessageRouting: { mode: "inherit" } },
      { id: "CodeReview", kind: "group", templateName: "CodeReview", groupRuleId: "group-rule:CodeReview", groupEnabled: true, initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
      { source: "Build", target: "UnitTest", trigger: "<default>", messageMode: "last" },
      { source: "Build", target: "TaskReview", trigger: "<default>", messageMode: "last" },
    ...
    groupRules: [
      {
        id: "group-rule:UnitTest",
        groupNodeName: "UnitTest",
        sourceTemplateName: "Build",
        entryRole: "entry",
        members: [{ role: "entry", templateName: "UnitTest" }],
        edges: [],
        exitWhen: "one_side_agrees",
        report: {
          sourceRole: "entry",
      ...
      {
        id: "group-rule:TaskReview",
        groupNodeName: "TaskReview",
        sourceTemplateName: "Build",
        entryRole: "entry",
        members: [{ role: "entry", templateName: "TaskReview" }],
        edges: [],
        exitWhen: "one_side_agrees",
        report: {
          sourceRole: "entry",
      ...
      {
        id: "group-rule:CodeReview",
        groupNodeName: "CodeReview",
        sourceTemplateName: "Build",
        entryRole: "entry",
        members: [{ role: "entry", templateName: "CodeReview" }],
        edges: [],
        exitWhen: "one_side_agrees",
        report: {
          sourceRole: "entry",
  ...
  assert.equal(afterBuild.state.groupBundles.length, 3);
});

test("线索发现没有 finding 并命中结束 trigger 时，会先转给线索完备性评估而不是继续触发 group decisionAgent", () => {
  ...
  assert.equal(afterTriage.state.groupBundles.length, 0);
});

test("线索发现存在 finding 且未发出 TASK_DONE 时，会按条件分支继续触发 group decisionAgent", () => {
  const topology = compileBuiltinTopology("vulnerability.json5").topology;
  const state = createGraphTaskState({
...
test("group 展开后，handoff 批次会把待响应目标同步成运行时实例 id，而不是残留静态 group 节点", () => {
  const topology: TopologyRecord = {
    nodes: ["线索发现", "辩论", "漏洞论证", "讨论总结"],
    nodeRecords: [
      { id: "线索发现", kind: "agent", templateName: "线索发现", initialMessageRouting: { mode: "inherit" } },
      { id: "辩论", kind: "group", templateName: "辩论", groupRuleId: "group-rule:辩论", groupEnabled: true, initialMessageRouting: { mode: "inherit" } },
      { id: "漏洞论证", kind: "agent", templateName: "漏洞论证", initialMessageRouting: { mode: "inherit" } },
      { id: "讨论总结", kind: "agent", templateName: "讨论总结", initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
  ...
  assertGroupRulesValid(topology);
  const state = createGraphTaskState({
    taskId: "task-group-batch-runtime-targets",
    topology,
  });
...
test("group 子图全部完成后，会把 group 节点视为完成并按普通 handoff 边继续流转", () => {
  const topology: TopologyRecord = {
    nodes: ["线索发现", "辩论", "漏洞论证", "讨论总结"],
    nodeRecords: [
      { id: "线索发现", kind: "agent", templateName: "线索发现", initialMessageRouting: { mode: "inherit" } },
...
test("all_completed 特殊补发会按 trigger 精确挑选匹配目标", () => {
  const topology: TopologyRecord = {
    ...
    nodeRecords: [
      ...
      { id: "证据补查", kind: "agent", templateName: "证据补查", initialMessageRouting: { mode: "inherit" } },
      { id: "总结甲", kind: "agent", templateName: "总结甲", initialMessageRouting: { mode: "inherit" } },
      { id: "总结乙", kind: "agent", templateName: "总结乙", initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
      { source: "线索发现", target: "辩论", trigger: "<default>", messageMode: "last" },
    ],
...
test("裁决直接回流到外层节点时，也会同步把 group 激活标记完成，避免后续卡住", () => {
  const topology: TopologyRecord = {
    nodes: ["线索发现", "辩论", "漏洞论证", "讨论总结"],
    nodeRecords: [
      { id: "线索发现", kind: "agent", templateName: "线索发现", initialMessageRouting: { mode: "inherit" } },
  ...
  assertGroupRulesValid(topology);
  const state = createGraphTaskState({
    taskId: "task-group-summary-report",
    topology,
  });
  ...
  assert.equal(afterSummary.decision.batch.sourceAgentId, "讨论总结-1");
  assert.deepEqual(afterSummary.decision.batch.jobs.map((job) => job.agentId), ["线索发现"]);
  assert.equal(afterSummary.state.groupActivations[0]?.dispatched, true);
  assert.equal(afterSummary.state.agentStatusesByName["辩论"], "completed");
});

test("单条 finding 的 group 通过 report 回外层时，会在总结完成后标记 activation 完成", () => {
  const topology: TopologyRecord = {
    nodes: ["辩论", "归档"],
    nodeRecords: [
      { id: "辩论", kind: "group", templateName: "辩论", groupRuleId: "group-rule:辩论", initialMessageRouting: { mode: "inherit" } },
      { id: "归档", kind: "agent", templateName: "归档", initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
  ...
  assertGroupRulesValid(topology);
  const state = createGraphTaskState({
    taskId: "task-single-item-group-report",
    topology,
  });
