...
test("回流超限时，如果 reviewer 正文已包含最终结论提示，不应再重复追加一遍", () => {
  const topology: TopologyRecord = {
    ...
    edges: [
      { source: "Build", target: "反方-1", triggerOn: "association" },
      { source: "反方-1", target: "Build", triggerOn: "needs_revision", maxRevisionRounds: 1 },
      { source: "反方-1", target: "裁决总结-1", triggerOn: "approved" },
    ],
  };
  let state = createGraphTaskState({
