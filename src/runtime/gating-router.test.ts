...
test("isExecutionDecisionAgent 会把 spawn 子图里带结束 trigger 出边的运行时实例识别为 decision agent", () => {
...
test("自动 handoff 命中 spawn 节点时，会实例化动态团队并派发入口角色，而不是停在 spawn 模板节点", () => {
  const topology: TopologyRecord = {
    ...
    nodeRecords: [
      ...
      { id: "UnitTest", kind: "spawn", templateName: "UnitTest", spawnRuleId: "spawn-rule:UnitTest", spawnEnabled: true, initialMessageRouting: { mode: "inherit" } },
      { id: "TaskReview", kind: "spawn", templateName: "TaskReview", spawnRuleId: "spawn-rule:TaskReview", spawnEnabled: true, initialMessageRouting: { mode: "inherit" } },
      { id: "CodeReview", kind: "spawn", templateName: "CodeReview", spawnRuleId: "spawn-rule:CodeReview", spawnEnabled: true, initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
      { source: "Build", target: "UnitTest", trigger: "<default>", messageMode: "last" },
      { source: "Build", target: "TaskReview", trigger: "<default>", messageMode: "last" },
...
test("spawn 展开后，handoff 批次会把待响应目标同步成运行时实例 id，而不是残留静态 spawn 节点", () => {
  const topology: TopologyRecord = {
    ...
    nodeRecords: [
      { id: "线索发现", kind: "agent", templateName: "线索发现", initialMessageRouting: { mode: "inherit" } },
      { id: "辩论", kind: "spawn", templateName: "辩论", spawnRuleId: "spawn-rule:辩论", spawnEnabled: true, initialMessageRouting: { mode: "inherit" } },
      { id: "漏洞论证", kind: "agent", templateName: "漏洞论证", initialMessageRouting: { mode: "inherit" } },
      { id: "讨论总结", kind: "agent", templateName: "讨论总结", initialMessageRouting: { mode: "inherit" } },
    ],
    edges: [
...
test("单条 finding 的 spawn 通过 report 回外层时，会在总结完成后标记 activation 完成", () => {
  const topology: TopologyRecord = {
    nodes: ["辩论", "归档"],
    nodeRecords: [
      { id: "辩论", kind: "spawn", templateName: "辩论", spawnRuleId: "spawn-rule:辩论", initialMessageRouting: { mode: "inherit" } },
