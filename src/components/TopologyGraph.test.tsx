...
test("agent 历史记录区应尽量贴近外层边框，避免上下左右留白过大", () => {
  ...
  assert.match(TOPOLOGY_GRAPH_SOURCE, /className=\{`rounded-\[10px\] border px-2\.5 py-1\.5 text-left \$\{getHistoryItemClassName\(item\)\}`\}/);
  ...
  assert.doesNotMatch(TOPOLOGY_GRAPH_SOURCE, /className=\{`rounded-\[10px\] border px-3 py-2 text-left \$\{getHistoryItemClassName\(item\)\}`\}/);
});
...
test("点击拓扑里的单条历史消息后，会弹出完整内容弹窗", () => {
  ...
  assert.match(TOPOLOGY_GRAPH_SOURCE, /selectedHistoryItem\.item\.detail/);
