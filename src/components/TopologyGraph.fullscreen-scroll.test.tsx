...
test("单个 agent 全屏详情首次打开会贴到底部，后续追加消息会继续向下滚动，用户离开底部后不会被强行拉回", async () => {
  const initialTask = createTask([
    createAgentFinalMessage({
      id: "message-1",
      content: "第一版输出",
      timestamp: toUtcIsoTimestamp("2026-04-28T10:00:00.000Z"),
    }),
    createAgentFinalMessage({
      id: "message-2",
      content: "第二版输出",
      timestamp: toUtcIsoTimestamp("2026-04-28T10:02:00.000Z"),
    }),
  ]);
  ...
  try {
    ...
    await rendered.render(createTask([
      ...
      createAgentFinalMessage({
        id: "message-3",
        content: "第三版输出",
        timestamp: toUtcIsoTimestamp("2026-04-28T10:05:00.000Z"),
      }),
    ]));
    await act(async () => {
      await rendered.flushAnimationFrames();
    ...
    await rendered.render(createTask([
      ...
      createAgentFinalMessage({
        id: "message-4",
        content: "第四版输出",
        timestamp: toUtcIsoTimestamp("2026-04-28T10:08:00.000Z"),
      }),
    ]));
    await act(async () => {
      await rendered.flushAnimationFrames();
