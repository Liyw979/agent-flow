...
test("消息、团队、拓扑三块面板复用完全一致的头部样式", () => {
  ...
  assert.match(TOPOLOGY_GRAPH_SOURCE, /PANEL_SECTION_BODY_CLASS/);
...
test("消息、团队、拓扑三块面板内容区复用完全一致的上下间距，并且缩小为当前的一半", () => {
  ...
  assert.doesNotMatch(TOPOLOGY_GRAPH_SOURCE, /px-5 pb-5 pt-4/);
});
