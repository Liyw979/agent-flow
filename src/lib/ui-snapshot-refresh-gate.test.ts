...
test("较新的 ui snapshot 响应一旦已被接受，较旧响应必须被拒绝，避免把 UnitTest 运行中回滚成 BA 运行中", () => {
  ...
  assert.equal(rejectedOlder.latestAcceptedState.kind, "accepted");
});
...
test("ui snapshot 门禁允许首次响应和更大请求号通过，但拒绝相同请求号重复回写", () => {
  ...
  assert.equal(duplicatedRequest.latestAcceptedState.kind, "accepted");
...
test("较大的请求号若带回更旧的任务快照，必须被拒绝，避免把 BA 已完成和 Build 已启动回滚成旧画面", () => {
  ...
  assert.equal(rejectedSemanticallyOlder.latestAcceptedState.kind, "accepted");
});
...
test("较小请求号若晚返回且语义上更新，门禁仍必须接受，避免并发刷新把群聊卡在旧快照", () => {
  ...
  const acceptedFreshLowerRequest = decideUiSnapshotRefreshAcceptance({
    ...
    requestId: 8,
    latestAcceptedState: acceptedStaleHigherRequest.latestAcceptedState,
    payload: createUiSnapshotPayload({
      baStatus: "completed",
      unitTestStatus: "running",
      buildStatus: "idle",
  ...
  assert.equal(readAcceptedPayload(acceptedFreshLowerRequest.latestAcceptedState).task?.messages.length, 3);
  assert.equal(
    readAcceptedPayload(acceptedFreshLowerRequest.latestAcceptedState).task?.agents.find((agent) => agent.id === "UnitTest")?.status,
    "running",
  );
});
...
test("较小请求号若仅补齐 session 与 attach，也必须被视为语义更新并接受", () => {
  ...
  assert.equal(
    readAcceptedPayload(acceptedLowerRequestWithAttach.latestAcceptedState).task?.agents.find((agent) => agent.id === "BA")?.opencodeSessionId,
    "session-ba-2",
  );
  assert.equal(
    readAcceptedPayload(acceptedLowerRequestWithAttach.latestAcceptedState).task?.agents.find((agent) => agent.id === "BA")?.opencodeAttachBaseUrl,
    "http://localhost:4310",
  );
});
...
test("较小请求号若只把旧的非空 session 与 attach 换成另一组非空值，门禁必须拒绝，避免回退到不可证明更晚的连接状态", () => {
  ...
  assert.equal(rejectedLowerRequest.latestAcceptedState.kind, "accepted");
});
