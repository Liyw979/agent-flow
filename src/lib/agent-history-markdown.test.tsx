...
test("拓扑历史正文里的连续空段需要压成普通换行，避免中间额外空行", () => {
  assert.equal(
    compactAgentHistoryMarkdownContent("第一段\n\n第二段"),
    "第一段\n第二段",
  );

  const html = renderAgentHistoryDetailToStaticHtml("第一段\n\n第二段");

  assert.match(html, /<p>第一段<br\/>第二段<\/p>/);
  assert.doesNotMatch(html, /<br\/>\n/);
  assert.doesNotMatch(html, /<p>第一段<\/p>\s*<p>第二段<\/p>/);
});

test("拓扑历史里的标题单独一行时，需要保留一处换行，但不能再膨胀成空白段距", () => {
  const html = renderAgentHistoryDetailToStaticHtml("**Assessing security vulnerabilities**\n\nI need to determine whether there's a real vulnerability present.");

  assert.match(html, /<p><strong data-chat-markdown-role="strong">Assessing security vulnerabilities<\/strong><br\/>I need to determine whether there&#x27;s a real vulnerability present\.<\/p>/);
  assert.doesNotMatch(html, /<br\/>\n/);
  assert.doesNotMatch(html, /<p><strong[^>]*>Assessing security vulnerabilities<\/strong><\/p>\s*<p>I need to determine whether there&#x27;s a real vulnerability present\.<\/p>/);
});
