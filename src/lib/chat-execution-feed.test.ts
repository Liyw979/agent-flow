...
test("buildChatFeedItems 会为只有 runtime snapshot 的 spawn agent 补出运行中执行气泡", () => {
  const messages = [
    createMessage({
      id: "clue-final",
      sender: "线索发现",
  ...
  assert.deepEqual(
    feedItems.map((item) =>
      item.type === "message"
        ...
        : { type: item.type, state: item.state, agentId: item.agentId }),
    [
      { type: "message", sender: "线索发现" },
      { type: "execution", state: "running", agentId: "漏洞挑战-1" },
    ],
  );
  ...
  if (!runningExecution || runningExecution.type !== "execution" || runningExecution.state !== "running") {
    assert.fail("缺少 runtime-only 的执行气泡");
  }
  assert.deepEqual(
