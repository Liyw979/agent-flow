...
test("package.json 提供 macOS 打包入口", () => {
  assert.equal(
    ...
    "bun run build && bun run build:embedded-assets && bun build --compile --target bun-darwin-arm64 ./cli/index.ts --outfile ./dist/agent-team-macos-arm64",
  );
  assert.equal(
    PACKAGE_JSON.scripts?.["dist:mac-x64"],
    "bun run build && bun run build:embedded-assets && bun build --compile --target bun-darwin-x64 ./cli/index.ts --outfile ./dist/agent-team-macos-x64",
  );
});
