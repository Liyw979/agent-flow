...
test("Build 作为内置模板可按需写入 Project，并可像其他 Agent 一样删除", async () => {
  ...
  assert.equal(
    ...
    "{\"permission\":{\"write\":\"deny\",\"edit\":\"deny\",\"bash\":\"deny\",\"task\":\"deny\",\"patch\":\"deny\"},\"agent\":{\"build\":{\"mode\":\"primary\",\"permission\":{\"write\":\"ask\",\"edit\":\"ask\",\"bash\":\"ask\",\"task\":\"ask\",\"patch\":\"ask\"}}}}",
  );
...
test("Build 写入后会固定为唯一可写 Agent", async () => {
  ...
  assert.equal(
    typed.customAgentConfig.buildInjectedConfigContent(projectPath),
    "{\"permission\":{\"write\":\"deny\",\"edit\":\"deny\",\"bash\":\"deny\",\"task\":\"deny\",\"patch\":\"deny\"},\"agent\":{\"build\":{\"mode\":\"primary\",\"permission\":{\"write\":\"ask\",\"edit\":\"ask\",\"bash\":\"ask\",\"task\":\"ask\",\"patch\":\"ask\"}},\"BA\":{\"mode\":\"primary\",\"prompt\":\"你是 BA。\",\"permission\":{\"write\":\"deny\",\"edit\":\"deny\",\"bash\":\"deny\",\"task\":\"deny\",\"patch\":\"deny\"}}}}",
  );
});
