...
test("旧脚本约束下，用户首条直接 @reviewer 的场景无法表达", async () => {
  const script = [
  ...
  await assert.rejects(
    () =>
      assertSchedulerScript({
        topology: createTopology({
          projectId: "direct-review-loop-entry-legacy-rejected",
          startAgentId: "Build",
          nodes: ["安全负责人", "漏洞分析人员", "Build"],
          edges: [
    ...
    /第一条 user 消息的 @Agent 必须等于 topology\.startAgentId/u,
  );
});
