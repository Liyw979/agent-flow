...
test("reviewer 的回流消息即使被其他消息隔开，也应继续合并回原结果卡片", () => {
  ...
  const merged = mergeTaskChatMessages([
    ...
    createMessage({
      id: "task-review-action-required-request",
      sender: "TaskReview",
...
test("action-required-request 单独展示时会移除标签后再追加 mention", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      id: "action-required-request",
      content: formatActionRequiredRequestContent(
...
test("action-required-request 单独展示时也会移除孤立的结束标签后再追加 mention", () => {
  const merged = mergeTaskChatMessages([
    createMessage({
      id: "action-required-request",
  ...
  const merged = mergeTaskChatMessages([
    createMessage({
      ...
      meta: {
        kind: "agent-final",
        reviewDecision: "action_required",
        finalMessage: `审视不通过。\n\n${REVIEW_CONTINUE_END_LABEL}请补充实现依据。`,
      },
    }),
  ]);
