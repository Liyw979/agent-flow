...
function createDevelopmentGraphDsl() {
  return {
    ...
    links: [
      ["BA", "Build", "handoff"] as const,
      ["Build", "CodeReview", "handoff"] as const,
      ["Build", "UnitTest", "handoff"] as const,
      ["Build", "TaskReview", "handoff"] as const,
      ["CodeReview", "Build", "action_required"] as const,
      ["UnitTest", "Build", "action_required"] as const,
      ["TaskReview", "Build", "action_required"] as const,
    ],
  };
}
...
test("compileTeamDsl 支持把递归式图 DSL 编译成 agents + topology", () => {
  const compiled = compileTeamDsl({
    ...
    links: [
      ["BA", "Build", "handoff"],
      ["Build", "SecurityResearcher", "handoff"],
      ["SecurityResearcher", "Build", "action_required"],
    ],
  });
...
test("compileTeamDsl 使用 handoff/action_required 作为公开拓扑触发名", () => {
  const compiled = compileTeamDsl({
    entry: "正方",
    nodes: [
      {
  ...
  assert.deepEqual(compiled.topology.edges, [
    { source: "正方", target: "反方", triggerOn: "action_required", messageMode: "last" },
  ]);
});

test("compileTeamDsl 只输出 handoff/action_required 这组公开拓扑触发名", () => {
  const compiled = compileTeamDsl({
    entry: "BA",
    nodes: [
      { type: "agent", name: "BA", prompt: BA_PROMPT },
      { type: "agent", name: "Build", writable: true },
      { type: "agent", name: "TaskReview", prompt: TASK_REVIEW_PROMPT },
    ],
    links: [
      ["BA", "Build", "handoff"],
    ...
    matchesAppliedTeamDslTopology(
      {
        ...
        edges: [{ source: "Build", target: "BA", triggerOn: "handoff" }],
      },
      compiled,
    ),
    false,
