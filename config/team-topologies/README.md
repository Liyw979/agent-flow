...
- 它只会在目标 Agent 的首次自动派发时生效，不会在同一个目标 Agent 的后续自动派发里重复注入。
...
- `讨论总结` 通过 `agent.initialMessage = ["漏洞论证", "漏洞挑战"]` 显式声明需要额外注入的来源，运行时会在该 Agent 首次自动派发时保留默认转发当前 source，并补入另一侧的首条可转发消息，同时对重复来源去重
