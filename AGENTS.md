...
- 只允许用户自定义 prompt；运行时会先全局把 `write / edit / bash / task / patch` 权限强制为 `deny`，再允许当前 Project 额外指定 0 个或 1 个“可写 Agent”恢复默认工具权限
...
- GUI 中点击 Agent 支持直接编辑并保存当前 Agent 的名称、prompt 与“可写 Agent”归属；“内置 Agent 模板”仍是可选项，不会自动写入当前 Project，可先单独编辑可编辑模板的 prompt 再按需写入为 Agent，且模板修改只影响当前 Project，不影响新项目默认值。Build 也作为默认模板提供，但它使用 OpenCode 自带 prompt，不支持在这里修改 prompt，仅支持按需写入当前 Project、删除已写入项，以及决定是否作为可写 Agent；同一个 Project 可以没有可写 Agent，但最多只能有一个；一旦当前 Project 出现 Task 启动记录，Agent 与内置模板配置都会被锁定，不允许继续修改
