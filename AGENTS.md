...
## 2. 功能地图（格式为文件名[方法列表], 对于我说的要求、功能要求、避免的要求要记录到这个章节）
...
- 团队拓扑 JSON5 只支持递归式 `entry + nodes + links` DSL；`links` 必须显式写出 `from / to / trigger / message_type`，其中 `trigger` 必须是尖括号包裹的字面值，例如 `<default>`、`<continue>`、`<complete>`、`<abcd>`。`agent` 节点可以额外声明 `initialMessage`，表示该目标 Agent 在接收自动派发时，需要按显式来源 Agent 列表传递首条可转发消息；空数组等同于 `none`。根图若需要显式结束来源，也可以继续通过 `{ "from": "...", "to": "__end__", "trigger": "<done>", "message_type": "none" }` 这类终止边表达。team-dsl[compileTeamDsl]
...
- 聊天区的动态执行窗口与拓扑运行态保持同步：除了用户消息、显式 `agent-dispatch`、`action-required-request` 触发的执行气泡外，当 `spawn` 产生的新 runtime agent 已经出现在 `runtimeSnapshots` 且正在运行、但最新 `uiSnapshot` 里还没有对应可见派发消息时，聊天区也必须立刻补出该 Agent 的动态窗口，避免出现“拓扑已开始运行、聊天仍需手动刷新才出现窗口”的分叉。chat-execution-feed[buildChatExecutionWindows, buildChatFeedItems]、agent-history[buildAgentExecutionHistoryItems]、App[App]
