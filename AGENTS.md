...
- Agent 自动触发下游 Agent 时，会封装 `[User Message]` 与带真实 Agent 名称的 `[<AgentName> Message]` 结构化段落；同时会在 `[Project Git Diff Summary]` 段附带当前 Project Git Diff 的精简摘要，帮助下游 Agent 快速感知最新改动
