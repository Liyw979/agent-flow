...
- 右上角拓扑图支持整块面板放大查看；放大视图会以近全屏方式优先展示拓扑本体，Agent 区与连线会优先贴近底部，上方留给消息记录；节点下游关系仍可通过点击节点编辑
- 拓扑图中的 Agent 节点顺序是稳定的：可在拓扑配置中指定一个“最左起点” Agent，默认优先取 `BA`；其余节点按累计上游依赖层级向右排列
...
- GUI 聊天区标题栏支持直接打开当前 Task 对应的 Zellij session，用于查看终端内实时运行状态
